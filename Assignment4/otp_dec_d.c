...
void handle_client(int client_fd, struct sockaddr_in peerinfo) {
    ...
    //decrypt the message
    char* decryptedmessage = decrypt(buff);

    // Send response
    send(client_fd, decryptedmessage, strlen(decryptedmessage), 0);
...
//decryption will take place in the process and ciphertext will be written back
//can either create new process every time a connection is made or set up a pool of 5 processes to handle decryption tasks
void otp_dec_d(int port)
{
...
int* map_letter_to_number(char* plaintext_sentence, int* size)
{
    ...
    *size = count;  // Update the size of the array
    ...
    // Reallocate memory to remove unused space
...
char* decrypt(char* buff)
{
    ...
    if (size_key >= size_message )
    {
        ...
        {
            //add the numbers then take the mod 27 to get the decrypted number
            int sumint = mapped_message[i] - mapped_key[i];
            int mod = (sumint % 27 + 27) % 27;
            //add the mod to an array of decrypted numbers then increment the pointer in array
            sum[sumptr] = mod;
            sumptr ++;
        }
        
        //iterate through the numbers and map the decrypted numbers back to letters
        //if it is 26, make it a space
